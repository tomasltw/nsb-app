...
const tag = (c) => ({
  ...
  background: c==="red"?"#e53e3e22": c==="green"?"#22c55e22":"#f9731622",
  color: c==="red"?"#e53e3e": c==="green"?"#22c55e":"#f97316",
  border:`1px solid ${c==="red"?"#e53e3e44": c==="green"?"#22c55e44":"#f9731644"}`
});
...
const base = {
  ...
  redBtn:{ background:"#e53e3e", color:"#fff", border:"none", padding:"14px 24px", borderRadius:10, cursor:"pointer", fontFamily:F, fontSize:15, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", width:"100%", display:"block" },
  ...
  navItem:(active)=>({ flex:1, display:"flex", flexDirection:"column", alignItems:"center", padding:"10px 0 12px", cursor:"pointer", color:active?"#e53e3e":"#555", fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em", border:"none", background:"transparent", fontFamily:F }),
};
  ...
  if (!user) return (
    <div style={{ minHeight:"100vh", background:"#0a0a0a", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"20px", fontFamily:F }}>
      ...
      <div style={{ background:"#111", border:"none", borderRadius:16, padding:"28px 20px", width:"100%", maxWidth:380 }}>
        ...
        {err && <p style={{ color:"#e53e3e", fontSize:13, marginBottom:12, fontFamily:F }}>{err}</p>}
        ...
        <div style={base.main}>
          ...
          {view==="home" && <>
            ...
            <div style={base.grid2}>
              <div style={base.statCard("#e53e3e")}><p style={base.h3}>Atletas</p><p style={{ fontSize:40, fontWeight:800, color:"#e53e3e", lineHeight:1, fontFamily:F }}>{athletes.length}</p></div>
          ...
          {view==="atletas" && <>
            ...
            {athletes.map(a=>{
              ...
              return <div key={a.id} style={base.card}>
                ...
                <button style={{...base.ghostBtn, marginTop:12, width:"100%", fontSize:12, color:"#e53e3e", borderColor:"#e53e3e44"}} onClick={()=>delAthlete(a.id)}>Eliminar atleta</button>
              </div>;
            })}
          </>}
            ...
            {workouts.length===0 ? <div style={base.card}><p style={{ color:"#444", fontFamily:F }}>Sin entrenamientos aún.</p></div> : workouts.map(w=>(
              <div key={w.id} style={base.card}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                  <div><p style={{ fontWeight:800, fontSize:16, color:"#e53e3e", textTransform:"uppercase", fontFamily:F }}>{w.title}</p><p style={{ color:"#666", fontSize:13, fontFamily:F }}>{w.users?.name} · {w.date}</p></div>
          ...
          {view==="horarios" && <>
            ...
            {schedules.map(sch=>(
              <div key={sch.id} style={base.card}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                  <div><p style={{ fontWeight:800, fontSize:18, textTransform:"uppercase", fontFamily:F }}>{sch.day}</p><p style={{ color:"#e53e3e", fontSize:28, fontWeight:800, lineHeight:1, fontFamily:F }}>{sch.time}</p></div>
                ...
                <button style={{...base.ghostBtn, marginTop:10, width:"100%", fontSize:12, color:"#e53e3e", borderColor:"#e53e3e44"}} onClick={()=>delSchedule(sch.id)}>Eliminar</button>
              </div>
            ))}
          </>}
              ...
              {messages.slice().reverse().map(m=>(
                <div key={m.id} style={{ padding:"10px 0", borderBottom:"1px solid #1a1a1a" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    <span style={{ fontWeight:700, fontSize:12, color:"#e53e3e", fontFamily:F }}>{m.from?.name} → {m.to?.name}</span>
  ...
  return (
    <div style={base.app}>
      ...
      <div style={base.main}>
        ...
        {view==="home" && <>
          <h1 style={base.h1}>Hola, <span style={{ color:"#e53e3e" }}>{user.name.split(" ")[0]}</span></h1>
          <p style={{ color:"#666", fontSize:14, marginBottom:16, fontFamily:F }}>Never Stop Building</p>
          {daysLeft<15 && <div style={{ background:"#1a0505", border:"1px solid #e53e3e", borderRadius:12, padding:14, marginBottom:12 }}>
            <p style={{ color:"#e53e3e", fontWeight:700, fontSize:14, fontFamily:F }}>⚠ Tu plan vence en {daysLeft} días</p>
          </div>}
          <div style={base.grid2}>
            <div style={base.statCard("#e53e3e")}><p style={base.h3}>Plan</p><p style={{ fontWeight:700, fontSize:14, fontFamily:F }}>{user.plan||"Sin plan"}</p><p style={{ color:"#f97316", fontSize:12, fontFamily:F }}>{user.type}</p></div>
            <div style={base.statCard("#f97316")}><p style={base.h3}>Vence en</p><p style={{ fontSize:36, fontWeight:800, color:daysLeft<15?"#e53e3e":daysLeft<30?"#f97316":"#22c55e", lineHeight:1, fontFamily:F }}>{daysLeft}</p><p style={{ color:"#666", fontSize:12, fontFamily:F }}>días</p></div>
          ...
          <div style={base.card}>
            ...
            {todayW ? <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <p style={{ fontWeight:800, fontSize:16, color:"#e53e3e", textTransform:"uppercase", fontFamily:F }}>{todayW.title}</p>
        ...
        {view==="plan" && <>
          ...
          <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:8, marginBottom:16 }}>
            {weekDates.map((d,i)=>{
              ...
              return <div key={key} onClick={()=>setSelDate(key)} style={{ minWidth:52, background:selDate===key?"#1a0a0a":"#161616", border:selDate===key?"1px solid #e53e3e":isToday?"1px solid #e53e3e44":"1px solid #222", borderRadius:12, padding:"10px 6px", textAlign:"center", cursor:"pointer" }}>
                <p style={{ color:"#666", fontSize:10, textTransform:"uppercase", fontFamily:F }}>{"LunMarMiéJueViéSábDom".slice(i*3,i*3+3)}</p>
                <p style={{ fontWeight:800, fontSize:20, color:isToday?"#e53e3e":"#fff", fontFamily:F }}>{d.getDate()}</p>
          ...
          <div style={base.card}>
            {workouts.find(w=>w.date===selDate) ? (()=>{
              ...
              return <>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                  <p style={{ fontWeight:800, fontSize:18, color:"#e53e3e", textTransform:"uppercase", fontFamily:F }}>{w.title}</p>
        ...
        {view==="agendar" && <>
          ...
          {schedules.map(sch=>{
            ...
            return <div key={sch.id} style={{...base.card, border:isMine?"1px solid #e53e3e":"1px solid #222", background:isMine?"#1a0a0a":"#161616"}}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                <div><p style={{ fontWeight:800, fontSize:18, textTransform:"uppercase", fontFamily:F }}>{sch.day}</p><p style={{ color:"#e53e3e", fontSize:28, fontWeight:800, lineHeight:1, fontFamily:F }}>{sch.time}</p></div>
        ...
        {view==="mensajes" && <>
          ...
          <div style={base.card}>
            ...
            {messages.length===0 ? <p style={{ color:"#444", fontFamily:F }}>Sin mensajes aún.</p> : messages.slice().reverse().map(m=>(
              <div key={m.id} style={{ padding:"10px 0", borderBottom:"1px solid #1a1a1a" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontWeight:700, fontSize:12, color:m.from_id===user.id?"#f97316":"#e53e3e", fontFamily:F }}>{m.from?.name}</span>
                  <span style={{ color:"#444", fontSize:11, fontFamily:F }}>{new Date(m.created_at).toLocaleDateString()}</span>
                </div>
                <p style={{ color:"#bbb", fontSize:14, fontFamily:F }}>{m.text}</p>
              </div>
